...
//		public static TextWriter Writer = Console.Out;
	...
	internal class NvdlDispatcher
	{
		...
		Stack<NvdlSection> sectionStack = new Stack<NvdlSection> ();
		...
		// trigger element
		Stack<XmlQualifiedName> qnameStack = new Stack<XmlQualifiedName> ();
...
Reader.Name, sectionStack.Count);
	...
	internal class NvdlSection
	{
		...
		readonly string ns;
		readonly IList<NvdlInterpretation> ilist = new List<NvdlInterpretation> ();
		readonly Stack<string> elementNameStack = new Stack<string> ();
		...
		public IList<NvdlInterpretation> Interpretations {
			get { return ilist; }
		}
		...
		public void StartElement ()
		{
			...
			if (!Reader.IsEmptyElement)
				elementNameStack.Push (Reader.LocalName);
		}
